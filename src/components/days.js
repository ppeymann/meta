...
const Dayes = (item) => {
  ...
  const activator = item.activator;
  const index = item.indexs;
  ...
  return (
    <div
      className={` rounded-lg text-gray-700 transition-all delay-75 flex flex-col justify-center items-center cursor-pointer px-2 py-3.5 hover:bg-green-500 hover:text-white  ${
        ...
        activator === index ? "!bg-green-500 !text-white" : ""
      } `}
    >
      ...
      <div
        className={`w-full delay-75 transition-all ease-linear rounded-lg mt-2 h-1 ${
